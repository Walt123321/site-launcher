<?php
if (!isset($jsToken)) { if (empty($_SESSION['js_token'])) { $_SESSION['js_token'] = bin2hex(random_bytes(16)); } $jsToken = $_SESSION['js_token']; }
...
if (!isset($asset_url)) { $asset_url = (PHP_SAPI === 'cli-server') ? '' : ($site_url . '/lander/' . $site_domain); }
?>
    ...
    function appendAgentMessage(text) {
        ...
        msg.innerHTML = `
            <div style="width: 28px; height: 28px; border-radius: 50%; border: 1px solid #e3ff34; background-color: #183c3e; display: flex; align-items: center; justify-content: center; overflow: hidden; flex-shrink: 0; margin-top: 2px;">
                <img src="<?= $asset_url ?>/consultant.webp" onerror="this.style.display='none'; this.nextElementSibling.style.display='block';" style="width:100%; height:100%; object-fit:cover;">
