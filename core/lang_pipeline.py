...
def _set_php_var(content: str, var: str, value: str, numeric: bool) -> str:
    ...
    # (;\s*$) вимагало, щоб рядок закінчувався ОДРАЗУ після ";" -- рядок на
    # кшталт `$var = "x"; // коментар` цьому не відповідав (після ";" йде
    # текст коментаря, не просто пробіли до кінця рядка), regex не матчився
    # на цьому рядку взагалі, і функція тихо йшла в галузь "вставити нове
    # присвоєння після <?php" -- залишаючи ОРИГІНАЛЬНИЙ рядок (зі старим
    # значенням) недоторканим ДАЛІ у файлі. PHP виконує зверху вниз, тож те
    # старе присвоєння виконувалось ПІЗНІШЕ і тихо перезаписувало щойно
    # вставлене правильне значення назад. Саме так $country_name у T3
    # завжди показував статичний "United Kingdom" незалежно від реального
    # гео запуску -- живий баг, знайдений на скріншоті. Група 4 тепер явно
    # ловить необов'язковий "// ..." коментар в кінці рядка й зберігає
    # його в заміні, замість того щоб регекс просто не спрацьовував.
    pattern = re.compile(rf"(^\s*\${re.escape(var)}\s*=\s*)(.*?)(;)([ \t]*(?://[^\n]*|#[^\n]*)?)\s*$", re.MULTILINE)
    if pattern.search(content):
        # ВАЖЛИВО: \g<1>, \g<3>, \g<4>, щоб числа не ламали group refs
        return pattern.sub(rf"\g<1>{rhs}\g<3>\g<4>", content, count=1)
